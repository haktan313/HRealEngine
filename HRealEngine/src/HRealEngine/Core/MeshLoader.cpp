...
{
    std::unordered_map<std::filesystem::path, Ref<MeshGPU>> MeshLoader::s_Cache;
    ...
    static bool ExtractAndCopyTexture(const aiScene* scene, const aiMaterial* mat, aiTextureType type, const std::filesystem::path& modelAbs,
        const std::filesystem::path& assetsRoot, const std::filesystem::path& texDirAbs, AssetHandle& outHandle)
    {
         outHandle = 0;
        if (!scene || !mat)
            return false;
        ...
        if (ref[0] == '*')
        {
            ...
            std::string ext = "png";
            if (emb->achFormatHint[0] != 0)
                ext = ToLower(std::string(emb->achFormatHint));

            std::filesystem::create_directories(texDirAbs);
            std::filesystem::path outAbs = texDirAbs / (std::string("embedded_") + std::to_string(idx) + "." + ext);
            
            if (emb->mHeight == 0)
            {
                std::ofstream f(outAbs, std::ios::binary);
                f.write(reinterpret_cast<const char*>(emb->pcData), emb->mWidth);
            }
            else
                return false;

            Project::GetActive()->GetEditorAssetManager()->ImportAsset(outAbs);
            auto rel = std::filesystem::relative(outAbs, assetsRoot);
            outHandle = Project::GetActive()->GetEditorAssetManager()->GetHandleFromPath(rel);
            return outHandle != 0;
        }
        
        std::filesystem::path texAbs;
        if (!MeshLoader::TryResolveTexturePath(modelAbs, ref, texAbs))
            return false;

        std::filesystem::create_directories(texDirAbs);
        std::filesystem::path outAbs = texDirAbs / texAbs.filename();

        if (!MeshLoader::CopyFileSafe(texAbs, outAbs))
            return false;
    ...
    std::vector<std::string> MeshLoader::ImportObjMaterialsToHMat(const std::filesystem::path& objPathInAssets,
        const std::filesystem::path& assetsRoot, const std::filesystem::path& lastCopiedTexAbs, const std::vector<std::filesystem::path>& texturePaths)
    {
        ...
        for (unsigned i = 0; i < scene->mNumMaterials; ++i)
        {
            ...
            ExtractAndCopyTexture(scene, mat, aiTextureType_DIFFUSE, objAbs, assetsRoot, texDir, albedoHandle);
            ExtractAndCopyTexture(scene, mat, aiTextureType_NORMALS, objAbs, assetsRoot, texDir, normalHandle);
            if (normalHandle == 0)
                ExtractAndCopyTexture(scene, mat, aiTextureType_HEIGHT, objAbs, assetsRoot, texDir, normalHandle);

            if (albedoHandle == 0)
            {
                if (!texturePaths.empty() && i - 1 < texturePaths.size())
                {
                    auto rel = std::filesystem::relative(texturePaths[i - 1], assetsRoot).generic_string();
                    albedoHandle = Project::GetActive()->GetEditorAssetManager()->GetHandleFromPath(std::filesystem::path(rel));
                }
            }
            ...
            hm << "NormalTextureHandle: "   << (uint64_t)normalHandle  << "\n";
            hm << "Shininess: " << 32.0f << "\n";
            hm.close();
    ...
    bool MeshLoader::WriteHMeshBin(const std::filesystem::path& path, const std::vector<MeshVertex>& vertices,
    ...
    Ref<MeshGPU> MeshLoader::GetOrLoad(const std::filesystem::path& hmeshPath, const std::filesystem::path& assetsRoot,
    ...
    bool MeshLoader::ParseHMeshMaterials(const std::filesystem::path& hmeshAbs, std::vector<std::string>& outMaterials)
    {
        ...
        bool MeshLoader::ParseHMeshMaterialHandles(const std::filesystem::path& hmeshAbs, /*std::vector<std::string>& outMaterials*/std::vector<AssetHandle>& outHandles)
    {
        std::ifstream in(hmeshAbs);
        if (!in)
            return false;
    ...
    Ref<MeshGPU> MeshLoader::LoadHMeshAsset(const std::filesystem::path& hmeshPath, const std::filesystem::path& assetsRoot,
        const Ref<Shader>& shader)
    {
    ...
    bool MeshLoader::ExtractCookedRelativePath(const std::filesystem::path& hmeshPath, std::string& outCookedRel)
    {
    ...
    bool MeshLoader::TryResolveTexturePath(const std::filesystem::path& objAbs, const std::string& texRelOrAbs,
    ...
    std::string MeshLoader::SanitizeName(std::string s)
    {
        for (char& c : s)
        {
            const bool ok =
    ...
    std::string MeshLoader::Trim(std::string s)
    {
    ...
    std::string MeshLoader::StripQuotes(std::string s)
    {
        s = Trim(std::move(s));
        if (s.size() >= 2)
        {
    ...
    std::string MeshLoader::RemainderAfterKeyword(const std::string& line, const std::string& keyword)
    {
        std::string rest = line.substr(keyword.size());
        return Trim(rest);
    }

    bool MeshLoader::StartsWith(const std::string& s, const char* prefix)
    {
        return s.rfind(prefix, 0) == 0;
    }

    void MeshLoader::NormalizeSlashes(std::string& s)
    {
        for (char& c : s) if (c == '\\') c = '/';
    }
    
    std::vector<std::string> MeshLoader::ParseObjMtllibs(const std::filesystem::path& objAbs)
    {
    ...
    bool MeshLoader::ExtractMtlMapPath(const std::string& line, std::string& outPath)
    {
    ...
    bool MeshLoader::RewriteMtlAndCollectTextures(const std::filesystem::path& srcMtlAbs, const std::filesystem::path& dstMtlAbs, std::vector<std::string>& outTextureRelOrAbs)
    {
    ...
    bool MeshLoader::CopyFileSafe(const std::filesystem::path& src, const std::filesystem::path& dst)
    {
        std::error_code ec;
